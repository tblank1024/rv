...
def main():
    if debug_level == 5:
        jsondumpfile = open('datalog.json', 'w')
    elif debug_level == 4:
        jsondumpfile = open('datalog.json', 'r')
    ...
    def getLine():
        if debug_level == 4:
            ...
            while True:
                try:
                    myresult_str = jsondumpfile.readline()
                    myresult = json.loads(myresult_str)
                except:
                    exit()
                curtime = float(myresult.get("timestamp"))
                if lasttime != 0:
                ...
                if screenOut>0:
                    print(topic,json.dumps(myresult,indent=2))
        ...
        message = q.get()
        if debug_level==1:
            print("{0:f} {1:X} ({2:X}) ".format(message.timestamp, message.arbitration_id, message.dlc),flush=True)
        ...
        try:
            ...
            myresult["timestamp"] = str(message.timestamp)
            if debug_level == 5:
                json.dump(myresult, jsondumpfile)
                jsondumpfile.write("\n")
