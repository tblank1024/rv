# -*- coding: utf-8 -*-
# bat2mqttV2.py - Fixed version with corrected data synchronization logic
# Based on bat2mqtt.py but using original Battery Bleak.py synchronization method
...
def notification_handler_battery(sender, data):
    ...
    ************  new record starts here
    ...
    # FIXED: Use original synchronization logic from Battery Bleak.py
    if data[0] == 0x0a:  # now sync'd to end of a record at start (ORIGINAL METHOD)
        if len(LastMessage) == 40:  # end of complete record with exactly 40 bytes (ORIGINAL METHOD)
            FieldData = LastMessage.split(",")
            CurTime = int(time.time())
            
            # FIXED: Use original voltage calculation (no division by 100)
            # The voltage field already contains the decimal point in the BLE data
            Volt    = float(FieldData[0])/100  # Keep this as it correctly handles decimal
            Temp    = int(FieldData[5])
            
            # FIXED: Use original current calculation (no multiplication by 2)
            # The 2x multiplier may have been added incorrectly
            Amps    = int(FieldData[7])  # Original method - no 2x multiplier
            #NOTE: positive amps => charging and negative amps => discharging
            
            Full    = int(FieldData[8])  # Battery percentage - this should now be accurate
        ...
        #Reset Vars (ORIGINAL METHOD)
        LastMessage = ""
    else:
        # FIXED: Use original message accumulation method
